...
void copy_file()
{
        ...
        else
        {
                do
                {
                        ...
                        newnode->link = NULL;
                        fscanf(fp,"%[^/]%*c%[^/]%*c%[^/]%*c%[^/]%*c%[^/]%*c%ld %f %f %f %f%*c",name,address,id,dept,uni,&mobile,&salary,&leave,&hrs,&rei);
...
void send_file()
{
        ...
        while(temp)
        {
                ...
                hrs = temp->hrs;
                rei = temp->rei;
                fprintf(fp,"\n%s/%s/%s/%s/%s/%ld %.2f %.1f %.2f %.2f",name,address,id,dept,uni,mobile,salary,leave,hrs,rei);
                temp = temp->link;
        }
        fclose(fp);
        remove("empdata.txt");
...
void delete_emp()
{
        ...
        {
                pr("\nEmployee with %s doesn't exist",iddel);
                return_menu();
        }
...
void disp_payslip()
{
        ...
        while(temp)
        {
                if(strcmp(temp->id,idprint) == 0)
                {
                        ...
                        pr("\nEPF           : %.2f",epf);
                        pr("\nETF           : %.2f",etf);
                        pr("\nUnion Charges : %.2f",uni_rate);
                        pr("\nLeave Charges : %.2f",leav_rate);
                        pr("\nOvertime Rate : %.2f",ot_rate);
                        pr("\nReimbursement : %.2f",rei);
